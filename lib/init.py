...
def has_ukkey_conf():
    ...
    valid_ukkey_conf = False

    # ensure ukkey_conf exists & readable
    #
    # if not, print a message stating that Ukkey Core must be installed and
    # configured, including JSONRPC access in ukkey.conf
    try:
        f = io.open(config.ukkey_conf)
        valid_ukkey_conf = True
    except IOError as e:
        print(e)

    return valid_ukkey_conf
...
def main():
    ...
    if not has_ukkey_conf():
        print("UkkeyCore must be installed and configured, including JSONRPC access in ukkey.conf")
        sys.exit(1)
