...
def is_valid_ukkey_address(address, network='mainnet'):
    ...
    ukkey_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Ukkey address
        return False

    if (address_version != ukkey_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('ukkey.conf')
