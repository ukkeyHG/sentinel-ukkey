...
sentinel_cfg = UkkeyConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_ukkeyd_proto_version_with_sentinel_ping = 70207


def get_ukkey_conf():
    if sys.platform == 'win32':
        ukkey_conf = os.path.join(os.getenv('APPDATA'), "UkkeyCore/ukkey.conf")
    else:
        home = os.environ.get('HOME')

        ukkey_conf = os.path.join(home, ".ukkeycore/ukkey.conf")
        if sys.platform == 'darwin':
            ukkey_conf = os.path.join(home, "Library/Application Support/UkkeyCore/ukkey.conf")

    ukkey_conf = sentinel_cfg.get('ukkey_conf', ukkey_conf)
