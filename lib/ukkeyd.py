...
class UkkeyDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some ukkeyd methods
    ...
    @classmethod
    def from_ukkey_conf(self, ukkey_dot_conf):
        from ukkey_config import UkkeyConfig
        config_text = UkkeyConfig.slurp_config_file(ukkey_dot_conf)
        creds = UkkeyConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = ukkeylib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = ukkeylib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = ukkeylib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_ukkeyd_proto_version_with_sentinel_ping)
